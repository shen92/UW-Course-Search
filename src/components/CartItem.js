...
function CartItem(props) {
  ...
  const onRemoveCourseButtonClick = () => {
    // Find course
    const courseIndex = cart.findIndex(cartCourse => cartCourse.key === course.key);
    if (courseIndex >= 0) {
      let newCart = [...cart];
  ...
  const onRemoveCourseSubSectionButtonClick = (sectionNumber, subsectionNumber) => {
    // Find course
    const courseIndex = cart.findIndex(cartCourse => cartCourse.key === course.key);
    if (courseIndex >= 0) {
      let newCart = [...cart];
      ...
      const sectionIndex = newCourseSections.findIndex(section => section.number === sectionNumber);
      if (sectionIndex >= 0) {
        let newCourseSectionSubsections = newCourseSections[sectionIndex].subsections;

        // Find subsection
        const subsectionIndex = 
          newCourseSectionSubsections.findIndex(subsection => subsection.number === subsectionNumber)
            ...
            <th>
              ...
              >
                ...
                <UWButton 
                  label="Remove"
                  variant="clean"
                  onClick={() => onRemoveCourseSectionButtonClick(section.number)}
                />
              </div>
            </th>
          </tr>
                ...
                <td>
                  ...
                  >
                    ...
                    <UWButton 
                      label="Remove" 
                      variant="clean"
                      onClick={() => onRemoveCourseSubSectionButtonClick(section.number, subsection.number)}
                    />
                  </div>
                </td>
              </tr>
  ...
  return (
    <div className="cartItem">
      ...
      <div>
        {renderCourseSections(course.sections)}
      </div>
      <UWButton
        label="Remove All"

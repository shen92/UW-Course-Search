...
function CourseCard(props) {
    ...
    const onAddCourseButtonClick = () => {
      ...
      setCart(newCart);
      setModalMessage(`${course.number} ${course.name} has been added to your cart.`);
      setShowMessageModal(true);
    }

    const onAddSectionButtonClick = (sectionNumber, sectionContent) => {
      let newCart = [...cart];
      const courseIndex = newCart.findIndex(cartCourse => course.key === cartCourse.key);
      ...
      if (courseIndex < 0) {
        ...
        const sectionIndex = newCourseSections.findIndex(section => section.number === sectionNumber);
        if (sectionIndex < 0) {
          newCourseSections.push(newSection);
        }
      }
      setCart(newCart);
      setModalMessage(`${course.number} ${course.name} ${sectionNumber} has been added to your cart.`);
      setShowMessageModal(true);
    }

    const onAddSubsectionButtonClick = (sectionNumber, sectionContent, subsectionNumber) => {
      let newCart = [...cart];
      const courseIndex = newCart.findIndex(cartCourse => course.key === cartCourse.key);
      ...
      if (sectionContent.subsections) {
        for (const subsection of Object.entries(sectionContent.subsections)) {
          subsectionNumber === subsection[0] && newSubsections.push(
            {
              number: subsection[0]
            }
          );
      ...
      if (courseIndex < 0) {
        ...
        if (sectionIndex < 0) {
          ...
          const subsectionIndex = newSubsections.findIndex(subsection => subsection.number === subsectionNumber);
      ...
      setCart(newCart);
      setModalMessage(`${course.number} ${course.name} ${sectionNumber} ${subsectionNumber} has been added to your cart.`);
      setShowMessageModal(true);
    }
          ...
          <Card key={index}>
            ...
            >
              ...
              <UWButton 
                label="Add to cart" 
                variant="clean"
                onClick={() => onAddSectionButtonClick(sectionNumber, sectionContent)}
              />
            </Card.Header>
          ...
          <Card key={index}>
            ...
            >
              ...
              <UWButton 
                label="Add to cart" 
                variant="clean"
                onClick={() => onAddSubsectionButtonClick(sectionNumber, sectionContent, subSectionNumber)}
              />
            </Card.Header>
            <Accordion.Collapse eventKey={index}>
              <Card.Body style={{ padding: 12 }}>

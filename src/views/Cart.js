...
function Cart(props) {
  ...
  const getTotalCredits = () => {
    return cart.length < 2 ? 
      cart.length === 1 ? cart[0].credits : 0 : 
    ...
    <TabContent>
      <div style={{ height: '100%', padding: 8, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{
          ...
          >
            {cart.length > 0 ? 
              cart.map(course => <CartItem key={course.number} course={course} cart={cart} setCart={setCart}/>) : 
              <div style={{fontSize: 18, color: 'darkgray'}}>Your cart is empty.</div>
            }
          </div>
